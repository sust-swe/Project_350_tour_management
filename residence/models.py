...
class Space(models.Model):
    ...
    shared = models.BooleanField(default=False, choices=[(False, 'No'), (True, 'Yes')])
    img = models.ImageField(upload_to='ResidenceSpacePhoto', null=True, blank=True, default=None)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['residence', 'space_type_name'], name='residence\'s unique space'),
...
class SpaceAvailable(models.Model):
    ...
    class Meta:
        constraints = [
            ...
            models.CheckConstraint(check=Q(avail_from__lte=F('avail_to')), name='sa valid date span ')
        ]
